  ...
  <view class="mb-2.5 px-4 bg-#fff">
    <BlockHeader :title="`记录${index}`" bgColor="white" barColor="#066FEC">
      <template v-slot:right>
        ...
        <text class="color-#F53F3f font-size-3.5" @click="del">删除</text>
      </template>
    </BlockHeader>
    <uv-form
    ...
    >
      ...
      <uv-form-item label="白海豚位置" prop="position" :required="true" borderBottom>
        <uv-input
      ...
      <!-- <uv-form-item label="图片上传（0/3）" prop="exsitDolphin" :required="true" borderBottom>
        <uv-upload
          :fileList="fileList"
          multiple
          :maxCount="3"
...
const { watchDate, isView, record, index } = toRefs(props)
const form = ref(null)
const rules = ref({
  startDate: {
    type: 'string',

...
<template>
  <view class="Box bg-#f5f5f5 h-100% py-2.5 overflow-auto">
    ...
    <view class="pt-4 pl-2.5 mx-2 bg-#fff border-rd-2 pb-5">
      <BlockHeader title="7日天气预报" bgColor="white" barColor="blue" class="mb-4"></BlockHeader>
      <view class="overflow-x-auto reportBox">
        <view
          v-for="(item, index) in weekWeather"
          :key="index"
          @click="routerTo(item)"
          :class="weatherType(item.weatherCode)"
          class="w-17 mr-1 h-60 border-rd-1 bg-#456CD0 p-3 flex flex-col flex-items-center"
        >
          <text class="color-#fff line-height-5.5 font-size-3.5 mb-3">{{ item.week }}</text>
          <text class="color-#fff line-height-5 font-size-3 mb-3">{{ item.date }}</text>
          <uv-image
            ...
            height="22px"
          ></uv-image>
          <text class="color-#fff line-height-5 font-size-3.5 mb-3 font-500">
            {{ item.minTemperature }}°
          </text>
          <text class="color-#fff line-height-5 font-size-3.5 mb-3 font-500">
            {{ item.maxTemperature }}°
          ...
          <uv-image
            :src="`/static/weather/img/windDirection.png`"
            width="22px"
            class="mb-3 windDirection"
            :customStyle="{ transform: `rotateZ(${180 + Number(item.windDirection)}deg)` }"
            height="22px"
          ></uv-image>
          <text class="color-#fff line-height-5 font-size-3.5 font-500">
            {{ item.windSpeed }}级
          </text>
        </view>
      </view>
